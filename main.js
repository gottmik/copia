...
function criarResposta(implantesCertos, alturaUser, larguraUser) {
  ...
  implantesCertos.map((implante) => {
    ...
    const alturas = getValueInRange(alturaUser, implante.altura, 1.5);
    const larguras = getValueInRange(larguraUser, implante.largura, 0.5);

    let medidas = [];

    alturas.forEach((altura) => {
      larguras.forEach((largura) => {
        medidas = [...medidas, ` ${altura} x ${largura}`];
    ...
    let medidasContainer = document.createElement('div');
    medidasContainer.className = 'medidasContainer';
    medidasContainer.innerHTML = `<div class='text-span'>${medidas}</div> `;


    let tipo = document.createElement('div');
    tipo.className = 'answerType';
    tipo.innerHTML = "<span class='text-span'>Osso: </span>" + implante.osso;
    ...
    value.innerHTML = "<span class='text-span'>Valor aproximado: </span>" + implante.valor;
    ...
    divAnswerContainer.appendChild(medidasContainer);
...
// const returnValueInRange = (userInput, array, difference) => {
//   return array.some(item => item >= userInput - difference && item <= userInput)
// }
